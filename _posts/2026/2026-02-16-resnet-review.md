...
층이 깊어지면서 활성화 함수로 인해 역전파 과정에서 기울기가 점점 사라지거나, 기울기가 기하급수적으로 커져 학습이 불가능하게 되어버리는 문제가 발생한다.  
이러한 문제들은 학습 초기부터 학습하는 것을 방해하지만, 논문에서는 Normalized Initialiation (He Init)과 Intermediate Normalization Layer (BatchNorm Layer)를 사용하여 해결하였다.

그렇다면 이제 기울기 소실/폭발 문제를 해결했으니 층을 깊게 쌓으면 표현력이 증가하니 복잡한 문제를 해결할 수 있지 않을까?  
하지만 실제로 층을 깊게 쌓아서 성능을 측정해본 결과 오히려 성능이 나빠지게 된다!

그렇다면 표현력이 너무 풍부해져서 Overfitting의 문제가 아닐까?  
...
이는 최적화가 잘 이루어지지 않았다는 것을 의미하며 결과적으로 모델의 깊이가 증가할수록 시스템의 학습 난이도가 어려워진다는 것을 시사한다.
...
BottleNeck Block의 경우 층이 깊어질 경우 연산량을 줄이기 위해 1\*1 Conv를 통해 차원을 줄이고 연산 후에 다시 증폭하는 과정을 포함했다.  
그리고 3\*3 Conv 연산을 한 번만 수행한다.
